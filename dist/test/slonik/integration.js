...
beforeEach(async (t) => {
    ...
    await pool0.connect(async (connection) => {
      ...
      WHERE
        ...
        datname = 'slonik_test'
    `);
        await connection.query((0, src_1.sql) `DROP DATABASE IF EXISTS ${src_1.sql.identifier([TEST_DATABASE_NAME])}`);
        await connection.query((0, src_1.sql) `CREATE DATABASE ${src_1.sql.identifier([TEST_DATABASE_NAME])}`);
    });
    await pool0.end();
...
afterEach(async (t) => {
    ...
    await pool.connect(async (connection) => {
      ...
      WHERE
        ...
        datname = 'slonik_test'
    `);
        await connection.query((0, src_1.sql) `DROP DATABASE IF EXISTS ${src_1.sql.identifier([t.context.testDatabaseName])}`);
    });
    await pool.end();
});
...
if (pgNativeBindingsAreAvailable) {
    ...
    `);
        await t.throwsAsync(pool.stream((0, src_1.sql) `
        SELECT name
        FROM person
      `, () => { }), {
