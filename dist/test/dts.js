...
const typeParserTypes = () => {
    ...
    (0, src_1.createPool)('postgres://', {
        typeParsers: [...(0, src_1.createTypeParserPreset)()],
    });
...
const sqlTypes = async () => {
  ...
  FROM (
    VALUES
    (
      ${src_1.sql.join([src_1.sql.join(['a1', 'b1', 'c1'], (0, src_1.sql) `, `), src_1.sql.join(['a2', 'b2', 'c2'], (0, src_1.sql) `, `)], (0, src_1.sql) `), (`)}
    )
  ) foo(a, b, c)
  WHERE foo.b IN (${src_1.sql.join(['c1', 'a2'], (0, src_1.sql) `, `)})
`);
    await connection.query((0, src_1.sql) `
...
const samplesFromDocs = async () => {
    ...
    `);
        await connection.query((0, src_1.sql) `
      SELECT (${src_1.sql.array([1, 2, 3], (0, src_1.sql) `int[]`)})
    `);
    };
    const sample3 = async () => {
        (0, src_1.sql) `SELECT id FROM foo WHERE id = ANY(${src_1.sql.array([1, 2, 3], 'int4')})`;
        (0, src_1.sql) `SELECT id FROM foo WHERE id != ALL(${src_1.sql.array([1, 2, 3], 'int4')})`;
    };
    const sample4 = async () => {
        await connection.query((0, src_1.sql) `
      SELECT bar, baz
